...
This module requires type stubs for ``networkx`` and ``scikit-learn`` when
running ``mypy --strict``. Install them with

```
python -m pip install types-networkx types-scikit-learn
```
...
# Install ``types-scikit-learn`` or run ``mypy --install-types`` to obtain
# type information for ``sklearn.manifold``. The ``type: ignore[import]``
# below suppresses the missing-stub error until stubs are installed.
